...
| customize your plots. We'll use the second workhorse function in the package, ggplot, as well as other graphing
| functions.
...
|===========                                                                                             |  10%
| There are 3 more. STATS are statistical transformations such as binning, quantiles, and smoothing which ggplot2
| applies to the data. SCALES show what coding an aesthetic map uses (for example, male = red, female = blue).
| Finally, the plots are depicted on a COORDINATE SYSTEM. When you use qplot these were taken care of for you.
...
| are what we want our aesthetics to represent so we enclose these as two arguments to the function aes. Try this
| now.
...
| different smoothing function. Use the up arrow to recover the expression you just typed, and instead of calling
| geom_smooth with no arguments, call it with the argument method set equal to the string "lm".

> g+geom_point()+ method = "lm"
Error in g + geom_point() + method = "lm" : could not find function "+<-"
> g+geom_point(), method = "lm"
Error: unexpected ',' in "g+geom_point(),"
...
| Now recall the expression you just used and add to it another call, this time to the function facet_grid. Use
| the formula . ~ drv as it argument. Note that this is the same type of formula used in the calls to qplot.
...
| is in the leftmost panel, front-wheel drive data is shown in the middle panel, and rear-wheel drive data in the
| rightmost. Notice that this is similar to the plot you created at the start of the lesson using qplot. (We used
| a different smoothing function than previously.)
...
| Type g+geom_point()+geom_smooth(method="lm") + facet_grid(.~drv)+ ggtitle("Swirl Rules!") at the command
| prompt.
...
| _smooth) has options to modify it. Also, the function theme() can be used to modify aspects of the entire plot,
| e.g. the position of the legend. Two standard appearance themes are included in ggplot. These are theme_gray()
| which is the default theme (gray background with white grid lines) and theme_bw() which is a plainer (black and
| white) color scheme.
...
| color equal to "pink", the argument size to 4, and the argument alpha to 1/2. Notice that all the arguments are
| set equal to constants.

> ggplot(g, geom_point(color = "pink", size = 4, alpha = 1/2))
Error in `ggplot()`:
  ! `mapping` must be created with `aes()`.
✖ You've supplied a <LayerInstance> object.
...
---
  Backtrace:
  ▆
1. ├─ggplot2::ggplot(g, geom_point(color = "pink", size = 4, alpha = 1/2))
2. └─ggplot2:::ggplot.default(g, geom_point(color = "pink", size = 4, alpha = 1/2))
...
| alpha equal to 1/2. The third is a call to the function aes with the argument color set equal to drv. Note that
| you MUST use the function aes since the color of the points is data dependent and not a constant as it was in
...
Error in aes(color = drv) + labs(title = "Swirl Rules!") : 
...
Error: unexpected ',' in "g + geom_point(color = drv) + labs(title = "Swirl Rules!") + x = "Displacement","
> skip()
...
| No more gray background! Also, if you have good eyesight, you'll notice that the font in the labels changed.
                 ...
                 |=================================================================                                       |  62%
                 | One final note before we go through a more complicated, layered ggplot example, and this concerns the limits of
                 | the axes. We're pointing this out to emphasize a subtle difference between ggplot and the base plotting
| function plot.
...
| We've created some random x and y data, called myx and myy, components of a dataframe called testdat. These
                 ...
                 | we set its y-value to be out of range of the other points. Use the base plotting function plot to create a line
                 | plot of this data. Call it with 4 arguments - myx, myy, type="l", and ylim=c(-3,3). The type="l" tells plot you
                 | want to display the data as a line instead of as a scatterplot.
...
| Create the graphical object g with a call to ggplot with 2 arguments, testdat (the data) and a call to aes with
| 2 arguments, x set equal to myx, and y set equal to myy.
...
| which isn't very noticeable. Now recall that at the beginning of the lesson we mentioned 7 components of a
                 | ggplot plot, one of which was a coordinate system. This is a situation where using a coordinate system would be
                 | helpful. Instead of adding ylim(-3,3) to the expression g+geom_line(), add a call to the function
...
| package. We'll continue to work with the mpg dataset.
                 ...
                 |====================================================================================                    |  81%
                 | Start by creating the graphical object g by assigning to it a call to ggplot with 2 arguments. The first is the
                 | dataset and the second is a call to the function aes. This call will have 3 arguments, x set equal to displ, y
                 | set equal to hwy, and color set equal to factor(year). This last will allow us to distinguish between the two
...
| A simple, yet comfortingly familiar scatterplot appears. Let's make our display a 2 dimensional multi-panel
                 ...
                 | arguments. The first is the formula drv~cyl, and the second is the argument margins set equal to TRUE. Try this
                 | now.
                 
                 > g+geom_point()+facet_grid(formula drv~cyl, margins = TRUE)
                 Error: unexpected symbol in "g+geom_point()+facet_grid(formula drv"
                 > g+geom_point()+ facet_grid(formula = drv~cyl, margins = TRUE)
                 Error in facet_grid(formula = drv ~ cyl, margins = TRUE) : 
                   unused argument (formula = drv ~ cyl)
                 ...
                 | so instead of seeing 3 rows (the number of drv factors) and 4 columns (the number of cyl factors) we see a 4 by
                 | 5 display. Note that the panel in position (4,5) is a tiny version of the scatterplot of the entire dataset.
