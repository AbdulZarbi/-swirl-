...
| only one variable of the data, so run the R command qplot with the first argument price and the argument data
| set equal to diamonds. This will show the frequency of different diamond prices.
...
> qplot(price, data = diamonds$price)
Error:
! `data` must be uniquely named but does not have names
Run `rlang::last_trace()` to see where the error occurred.
> qplot(price, data = diamonds, range = diamonds$price)
`stat_bin()` using `bins = 30`. Pick better value with `binwidth`.
Warning message:
In geom_histogram(range = c(326L, 326L, 327L, 334L, 335L, 336L,  :
  Ignoring unknown parameters: `range`
...
| of brk. For instance, the first count is the number of diamonds with prices between 0 and $617, and the second
| is the number of diamonds with prices between $617 and $1234. Look at the vector named counts now.

> vector(brk)
Error in vector(brk) : invalid 'mode' argument
> skip()
...
| You're the best!
    ...
    | cut of diamonds. It might be hard to see, but around each line is a shadow showing the 95% confidence interval.
...
| should be set equal to the formula .~cut. Recall that the facets argument indicates we want a multi-panel plot.
| The symbol to the left of the tilde indicates rows (in this case just one) and the symbol to the right of the
| tilde indicates columns (in this five, the number of cuts). Try this now.
...
`geom_smooth()` using formula = 'y ~ x'
Warning message:
In geom_smooth(method = "lm", facets = . ~ cut) :
...
Error: unexpected '=' in "cutpoints <- quantile(data(diamonds$carat)+ seq(0,1, length = 4) + na.rm ="
> cutpoints <- quantile(diamonds$carat)+ seq(0, 1, length = 4) + na.rm = TRUE
Error: object 'cutpoints' not found
> skip()
...
| size in the dataset and 5.01 is the largest. One third of the diamonds are between .2 and .5 carats and another
| third are between .5 and 1 carat in size. The remaining third are between 1 and 5.01 carats. Now we can use the
| R command cut to label each of the 53940 diamonds in the dataset as belonging to one of these 3 factors. Create
| a new name in diamonds, diamonds$car2 by assigning it the output of the call to cut. This command takes 2
| arguments, diamonds$carat, which is what we want to cut, and cutpoints, the places where we'll cut.
    
    > cutpoints <- quantile(diamonds$carat, diamonds$car2, seq(0,1,length=4),na.rm=TRUE)
    Error in names && np > 0L : 'length = 4' in coercion to 'logical(1)'
    In addition: Warning message:
      Unknown or uninitialised column: `car2`. 
    > skip()
    ...
    > g <- ggplot(data= diamonds, aes(depth, price) +geom_point(alpha = 1/3, facet_grid(~car2)))
    Error in `geom_point()`:
    ...
    3. │ └─ggplot2::is_mapping(mapping)
    4. └─ggplot2::geom_point(alpha = 1/3, facet_grid(~car2))
    ...
    | data points with missing data (NA or Not Available) occurred. We see that there were only a handful (12 in
                                                                                                           | fact) and they occurred in Very Good, Premium, and Ideal cuts. We created a vector, myd, containing the indices
    | of these datapoints. Look at these entries in diamonds by typing the expression diamonds[myd,]. The myd tells R
    | what rows to show and the empty column entry says to print all the columns.
...
| but the carat field shows they each had a carat size of .2. What's going on here?
    ...
    | each column, and we see that the first column is labeled (0.2,0.5]. This indicates that this column contains
| data greater than .2 and less than or equal to .5. So diamonds with carat size .2 were excluded from the car2
| field.
...
| call is to ggplot with 2 arguments, diamonds and a call to aes with carat and price as arguments. The second
| call is to geom_boxplot with no arguments. The third is to facet_grid with one argument, the formula . ~ cut.
| Try this now.

> ggplot(data = diamonds, aes(carat, price)+ geom_boxplot() + facet_grid(.~cut))
Error in `ggplot()`:
! `mapping` must be created with `aes()`.
✖ You've supplied NULL.
Run `rlang::last_trace()` to see where the error occurred.
> rlang::last_trace()
<error/rlang_error>
  Error in `ggplot()`:
...
| Type ggplot(diamonds,aes(carat,price))+geom_boxplot()+facet_grid(.~cut) at the command prompt.
