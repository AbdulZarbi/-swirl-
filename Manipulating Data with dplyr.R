...
|====                                                                               |   5%
| I've created a variable called path2csv, which contains the full file path to the dataset.
| Call read.csv() with two arguments, path2csv and stringsAsFactors = FALSE, and save the
| result in a new variable called mydf. Check ?read.csv if you need help.
...
| Normally, this is something you would have to do on your own. Just to build the habit, type library(dplyr) now to
...
| You are quite good my friend!
  ...
  |===========                                                                        |  13%
| If your dplyr version is not at least 0.4.0, then you should hit the Esc key now, reinstall dplyr, then resume
| this lesson where you left off.

...

|============                                                                       |  15%
| The first step of working with data in dplyr is to load the data into what the package authors call a 'data frame
| tbl' or 'tbl_df'. Use the following code to create a new tbl_df called cran:
  | 
  | cran <- tbl_df(mydf).
...
| Perseverance, that's the answer.
  ...
  |==============                                                                     |  17%
| To avoid confusion and keep things running smoothly, let's remove the original data frame from your workspace
| with rm("mydf").
...
| columns as fit neatly in our console. At the bottom, we see the names and classes for any variables that didn't
| fit on our screen.
...
| select(cran, ip_id, package, country) to select only the ip_id, package, and country variables from the cran
| dataset.
...
| normally would when referring to columns of a data frame. The select() function knows we are referring to columns
| of the cran dataset.

...

|==========================                                                         |  32%
| Also, note that the columns are returned to us in the order we specified, even though ip_id is the rightmost
| column in the original dataset.

...

|============================                                                       |  33%
| Recall that in R, the `:` operator provides a compact notation for creating a sequence of numbers. For example,
| try 5:20.
...
| That's correct!
  ...
  |================================                                                   |  38%
| Print the entire dataset again, just to remind yourself of what it looks like. You can do this at anytime during
| the lesson.
...
| Keep working like that and you'll get there!
  ...
  |=================================                                                  |  40%
| Instead of specifying the columns we want to keep, we can also specify the columns we want to throw away. To see
| how this works, do select(cran, -time) to omit the time column.
...
> size (x:size)
Error in size(x:size) : could not find function "size"
> -5:20
...
| Keep up the great work!
  ...
  |====================================                                               |  43%
| Oops! That gaves us a vector of numbers from -5 through 20, which is not what we want. Instead, we want to negate
| the entire sequence of numbers from 5 through 20, so that we get -5, -6, -7, ... , -18, -19, -20. Try the same
| thing, except surround 5:20 with parentheses so that R knows we want it to first come up with the sequence of
| numbers, then apply the negative sign to the whole thing.
...
...
  ...
  |===========================================                                        |  52%
| The == operator asks whether the thing on the left is equal to the thing on the right. If yes, then it returns
| TRUE. If no, then FALSE. In this case, package is an entire vector (column) of values, so package == "swirl"
| returns a vector of TRUEs and FALSEs. filter() then returns only the rows of cran corresponding to the TRUEs.

...

  |============================================                                       |  53%
| You can specify as many conditions as you want, separated by commas. For example filter(cran, r_version ==
| "3.1.1", country == "US") will return all rows of cran corresponding to downloads from users in the US running R
| version 3.1.1. Try it out.

> filter(cran, r_version == "3.2.2", country == "US")
# A tibble: 0 × 11
# ℹ 11 variables: X <int>, date <chr>, time <chr>, size <int>, r_version <chr>, r_arch <chr>, r_os <chr>,
#   package <chr>, version <chr>, country <chr>, ip_id <int>

| You're close...I can feel it! Try it again. Or, type info() for more options.

| filter(cran, r_version == "3.1.1", country == "US") will return all rows of cran corresponding to downloads from
| users in the US running R version 3.1.1. Don't forget the quotes around 3.1.1 and US!
...
| Nice work!
  ...
  |================================================                                   |  58%
| Our last two calls to filter() requested all rows for which some condition AND another condition were TRUE. We
| can also request rows for which EITHER one condition OR another condition are TRUE. For example, filter(cran,
| country == "US" | country == "IN") will gives us all rows for which the country variable equals either "US" or
| "IN". Give it a go.
...
| numeric) AND r_os equals "linux-gnu". Hint: You are passing three arguments to filter(): the name of the dataset,
| the first condition, and the second condition.

> filter(cran, > 100500, r_os == "linux-gnu")
Error: unexpected '>' in "filter(cran, >"
> filter(cran > 100500, r_os = "linux-gnu")
...
| That's correct!
    ...
    |===================================================                                |  62%
    | Finally, we want to get only the rows for which the r_version is not missing. R represents missing values with NA
    | and these missing values can be detected using the is.na() function.
  ...
  |======================================================                             |  65%
| Now, put an exclamation point (!) before is.na() to change all of the TRUEs to FALSEs and all of the FALSEs to
| TRUEs, thus telling us what is NOT NA: !is.na(c(3, 5, NA, 10)).
...
| We've seen how to select a subset of columns and rows from our dataset using select() and filter(), respectively.
    ...
    |==========================================================                         |  70%
    | Sometimes we want to order the rows of a dataset according to the values of a particular variable. This is the
    | job of arrange().
    
    ...
    
    |===========================================================                        |  72%
    | To see how arrange() works, let's first take a subset of cran. select() all columns from size through ip_id and
...
| You got it!
  ...
  |=============================================================                      |  73%
| Now, to order the ROWS of cran2 so that ip_id is in ascending order (from small to large), type arrange(cran2,
| ip_id). You may want to make your console wide enough so that you can see ip_id, which is the last column.
...
| ip_id) will first arrange by package names (ascending alphabetically), then by ip_id. This means that if there
| are multiple rows with the same value for package, they will be sorted by ip_id (ascending numerically). Try
...
| All that hard work is paying off!
  ...
  |=================================================================                  |  78%
| Arrange cran2 by the following three variables, in this order: country (ascending), r_version (descending), and
| ip_id (ascending).

> arrange(cran2, country, desc(r_verson), ip_id)
Error in `arrange()`:
ℹ In argument: `..2 = r_verson`.
Caused by error:
! object 'r_verson' not found
...
| That's correct!
      ...
      |==================================================================                 |  80%
    | To illustrate the next major function in dplyr, let's take another subset of our original data. Use select() to
| grab 3 columns from cran -- ip_id, package, and size (in that order) -- and store the result in a new variable
| called cran3.
...
| Nice try, but that's not exactly what I was hoping for. Try again. Or, type info() for more options.
    ...
    | cran3 <- select(cran, ip_id, package, size) will store just these three columns in a new variable called cran3.
...
| It's common to create a new variable based on the value of one or more variables already in a dataset. The
    ...
    | megabytes (MB) are a more common unit of measurement. One megabyte is equal to 2^20 bytes. That's 2 to the power
| of 20, which is approximately one million bytes!
...
| We want to add a column called size_mb that contains the download size in megabytes. Here's the code to do it:
    ...
    <int> <chr>          <int>   <dbl>   <dbl>
      1     1 htmltools      80589 0.0769    78.7 
    2     2 tseries       321767 0.307    314.  
    3     3 party         748063 0.713    731.  
    4     3 Hmisc         606104 0.578    592.  
    5     4 digest         79825 0.0761    78.0 
    6     3 randomForest   77681 0.0741    75.9 
    7     3 plyr          393754 0.376    385.  
    8     5 whisker        28216 0.0269    27.6 
    9     6 Rcpp            5928 0.00565    5.79
    10     7 hflights     2206029 2.10    2154.  
    # ℹ 225,458 more rows
    # ℹ Use `print(n = ...)` to see more rows
    
    | You almost had it, but not quite. Try again. Or, type info() for more options.
    
    | Use mutate(cran3, size_mb = size / 2^20, size_gb = size_mb / 2^10) to create two new columns called size_mb and
    | size_gb, respectively.
...
| You nailed it! Good job!
  ...
  |===============================================================================    |  95%
| That's not particularly interesting. summarize() is most useful when working with data that has been grouped by
    | the values of a particular variable.
    
    ...
    
    |================================================================================   |  97%
    | We'll look at grouped data in the next lesson, but the idea is that summarize() can give you the requested value
| FOR EACH group in your dataset.

...

  |================================================================================== |  98%
| In this lesson, you learned how to manipulate data using dplyr's five main functions. In the next lesson, we'll
| look at how to take advantage of some other useful features of dplyr to make your life as a data analyst much
| easier.
