...
| lesson corresponds to 04_ExploratoryAnalysis/PlottingLattice.)

...

  |=                                                                         |   1%
| In another lesson, we gave you an overview of the three plotting systems in R. In this lesson
| we'll focus on the lattice plotting system.  As we did with the base plotting system, we'll
| focus on using lattice to create graphics on the screen device rather than another graphics
| device.
...
...
  ...
  |===                                                                       |   4%
| Lattice is implemented using two packages. The first is called, not surprisingly, lattice, and
| it contains code for producing Trellis graphics. Some of the functions in this package are the
| higher level functions which you, the user, would call. These include xyplot, bwplot, and
| levelplot.
...
| That's a job well done!
  ...
  |======                                                                    |   7%
| The second package in the lattice system is grid which contains the low-level functions upon
| which the lattice package is built. You, the user, seldom call functions from the grid package
| directly.
...
| This indicates that y depends on x, so in a scatterplot y would be plotted on the y-axis and x
| on the x-axis.
...
| Remember when we said that lattice is good for plotting multivariate data? That's where these
...
| looked up.  If no data frame or list is passed, then the parent frame is used. If no other
| arguments are passed, the default values are used.

...

  |============                                                              |  16%
| Recall the airquality data we've used before. We've loaded it again for you. To remind yourself
| what it looks like run the R command head with airquality as an argument to see what the data
| looks like.
...
| That's a job well done!
  ...
  |==============                                                            |  19%
| Look vaguely familiar? The dots are blue, instead of black, but lattice labeled the axes for
| you. You can use some of the same graphical parameters (e.g., pch and col) that you used in the
...
> xyplot(Ozen~Wind | as.factor(Month), data = airquality, layout = c(5,1))
Error in eval(i, data, env) : object 'Ozen' not found
> xyplot(Ozen~Wind, as.factor(Month), data = airquality, layout = c(5,1))
...
> xyplot(as.factor(Ozone ~ Wind), data = airquality, layout = c(5,1))
Error in unique.default(x, nmax = nmax) : 
  unique() applies only to vectors
> xyplot(as.factor(Month), data = airquality, layout = c(5,1))
Error: object 'Month' not found
> skip()
...
| That's a job well done!
  ...
  |======================                                                    |  30%
| Not as informative, right? The word Month in each panel really doesn't tell you
| much if it doesn't identify which month it's plotting. Notice that the actual data
| is the same between the two plots, though.
...
| (e.g., screen, or file such as a PDF file). In contrast, lattice graphics
...
| on the graphics device. They return "plot objects" that can be stored (but it’s
                                                                         | usually better to just save the code and data). On the command line, trellis
| objects are auto-printed so that it appears the function is plotting the data.

...

|=========================                                                 |  34%
| To see this, create a variable p which is assigned the output of this simple call
| to xyplot, xyplot(Ozone~Wind,data=airquality).
...
> R(p)
Error in R(p) : could not find function "R"
> names(p)
...
| making multi-panel plots makes lattice very appealing. The lattice package comes
...
| - two 100-long vectors, x and y. For its first 50 values y is a function of x, for
| the last 50 values, y is random. We've also defined a 100-long factor vector f
...
| equal to c(2,1). Note that we're not providing an explicit data argument, so
| xyplot will look in the environment and see the x and y that we've generated for
| you.
...
| left panel (Group 1) in the plot. Now look at v2 which holds the comparable
| numbers for Group 2.
...
| That's a job well done!
  ...
  |========================================                                  |  54%
| Again, the values match the plot. That's reassuring. We've copied some code from
| the slides for you. To see it, type myedit("plot1.R"). This will open your editor
| and display the R code in it.
...
| The number of panels depends on the conditioning variable, and in this case it is
| f.
...
| Nice try, but that's not exactly what I was hoping for. Try again.
                ...
                | Recall the long call you just made to plot 35 panels in one picture. We would have
                | broken it up if we could have, but we didn't, so we can't.
...
| Nice try, but that's not exactly what I was hoping for. Try again.
                ...
                | This is the opposite of the last question which was false.
...
| object but base doesn't.
                ...
                | Recall our advice to NEVER trust questions with NEVER and ALWAYS in them.
...
| Congrats! We hope this lesson didn't leave you climbing the trellis.
                ...
                Selection: 2
                What is your email address? zarbi786@yahoo.com
                What is your assignment token? Mw2mLCNo9GcVoOeE
